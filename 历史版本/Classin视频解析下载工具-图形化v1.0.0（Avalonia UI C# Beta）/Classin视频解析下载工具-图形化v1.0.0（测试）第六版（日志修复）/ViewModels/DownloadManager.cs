...
namespace Classin视频解析下载工具.ViewModels
{
    public sealed class DownloadManager : INotifyPropertyChanged, IDisposable
    {
        ...
        private void AddToQueue(VideoItem item)
        {
            ...
            _loggingService.Debug($"队列信号已释放，当前队列大小: {_downloadQueue.Count}", "DownloadManager.Queue");
        }

        private void StartQueueMonitor()
        {
            _loggingService.Info("开始启动下载队列监控", "DownloadManager.Queue");
            Task.Run(MonitorDownloadQueue);
        }

        private async Task MonitorDownloadQueue()
        {
            _loggingService.Debug("下载队列监控任务已启动", "DownloadManager.Queue");
            ...
            while (!_queueCts.IsCancellationRequested)
            {
                try
                {
                    ...
                    while (_activeDownloadCount < MaxConcurrentDownloads)
                    {
                        ...
                        {
                            _loggingService.Trace("下载队列为空，等待新任务", "DownloadManager.Queue");
                            break;
                        }
                        ...
                        {
                            _loggingService.Debug($"从队列中取出下载任务: {item.Name}", "DownloadManager.Queue");
                            StartDownloadTask(item);
                        }
                        else
                        {
                            _loggingService.Debug($"跳过无效或已完成的下载任务: {item?.Name}", "DownloadManager.Queue");
                        }
                    }
                ...
                {
                    _loggingService.Info("下载队列监控已取消", "DownloadManager.Queue");
                    break;
                }
                catch (Exception ex)
                {
                    _loggingService.Error($"下载队列监控出现异常: {ex.Message}", "DownloadManager.Queue");
                    await Task.Delay(500, _queueCts.Token);
                }
            }
            
            _loggingService.Debug("下载队列监控任务结束", "DownloadManager.Queue");
        }
        ...
        private void StartDownloadTask(VideoItem item)
        {
            try
            {
                ...
                _loggingService.Debug($"活跃下载任务数增加至: {_activeDownloadCount}/{MaxConcurrentDownloads}", "DownloadManager.Task");
                ...
                var task = Task.Run(async () =>
                {
                    ...
                    finally
                    {
                        ...
                        _loggingService.Debug($"下载任务完成，活跃任务数减少至: {_activeDownloadCount}", "DownloadManager.Task");
                ...
                _loggingService.Debug($"下载任务已加入活跃任务列表，总任务数: {_activeDownloadTasks.Count}", "DownloadManager.Task");
            }
            catch (Exception ex)
            {
