...
namespace Classin视频解析下载工具.Services
{
    ...
    public interface IDownloadService : IDisposable
    {
        ...
        /// 检查是否有活跃的下载任务
        /// </summary>
        /// <returns>是否有活跃下载任务</returns>
        bool HasActiveDownloads();

        /// <summary>
        /// 获取活跃下载任务数量
        /// </summary>
        /// <returns>活跃下载任务数量</returns>
    ...
    public sealed class DownloadService : IDownloadService
    {
        ...
        public void Dispose()
        {
                ...
                try
                {
                    ...
                    lock (_staticLock)
                    {
                        foreach (var kvp in _activeDownloads)
                        {
                            ...
                            {
                                Console.WriteLine($"取消下载任务异常: {ex.Message}");
                            }
                        }
                    ...
                    {
                        Console.WriteLine($"清理HTTP客户端异常: {ex.Message}");
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"DownloadService清理异常: {ex.Message}");
                }
                finally
                {
                    _disposeSemaphore.Release();
            ...
            {
                Console.WriteLine($"DownloadService信号量操作异常: {ex.Message}");
            }
        }
