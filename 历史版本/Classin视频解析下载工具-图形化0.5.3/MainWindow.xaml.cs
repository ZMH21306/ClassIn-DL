...
namespace VideoDownloader
{
    ...
    public class IndexFormatConverter : IValueConverter
    {
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            {
                return index.ToString("D2"); // 格式化为2位数
            }
            return "00";
        }
    ...
    public partial class MainWindow : Window
    {
        ...
        {
            _logScrollToEnd = e.VerticalOffset == e.ExtentHeight - e.ViewportHeight;
        }

        private void OutputScrollViewer_ScrollChanged(object sender, ScrollChangedEventArgs e)
        {
            _outputScrollToEnd = e.VerticalOffset == e.ExtentHeight - e.ViewportHeight;
        }
        ...
        private async void DownloadSingle_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button button && button.Tag is VideoItem item)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    AppendDownloadOutput($"下载单个视频时出错: {ex.Message}");
                }
            }
        }
        ...
        private async Task DownloadVideosAsync()
        {
            try
            {
                ...
                if (downloadTasks.Count == 0)
                {
                    ...
                    {
                        progressBar.Value = 100;
                        txtCurrentAction.Text = "没有需要下载的任务";
                        btnDownload.IsEnabled = true;
                        btnParse.IsEnabled = true;
                        UpdateStatus("没有需要下载的视频", Brushes.Green);
                    });
                    return;
                }
        ...
        // 修复日志显示问题 - 确保每条日志只占一行且最后没有空行
        private void AppendLog(string message)
        {
            Dispatcher.Invoke(() =>
            {
                // 保存当前滚动位置状态
                bool wasAtEnd = _logScrollToEnd;
                ...
                {
                    // 检查最后字符是否是换行符，避免重复添加
                    if (!txtLog.Text.EndsWith(Environment.NewLine))
                    {
        ...
        // 修复下载输出显示问题 - 确保每条输出只占一行且最后没有空行
        private void AppendDownloadOutput(string message)
        {
            Dispatcher.Invoke(() =>
            {
                // 保存当前滚动位置状态
                bool wasAtEnd = _outputScrollToEnd;
                ...
                {
                    // 检查最后字符是否是换行符，避免重复添加
                    if (!txtDownloadOutput.Text.EndsWith(Environment.NewLine))
                    {
