...
namespace VideoDownloader
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private int _maxConcurrentDownloads = DEFAULT_MAX_CONCURRENT_DOWNLOADS;
            ...
            try
            {
                ...
                foreach (var item in videoItems)
                {
                    if (!string.IsNullOrEmpty(item.Url))
                    {
                        ...
                        batContent += $"aria2c.exe --allow-overwrite=true --file-allocation=none -d \"{downloadPath}\" -o \"{safeName}.mp4\" \"{item.Url}\"\r\n";
                    }
                }
        ...
        private async Task DownloadVideosAsync()
        {
            ...
            try
            {
                ...
                foreach (var item in videosToDownload.Where(i => i.Status == "等待下载"))
                {
                    string safeName = CleanName(item.Name);
                    string outputFile = Path.Combine(downloadPath, $"{safeName}.mp4");
        ...
        private async Task<bool> DownloadSingleVideoAsync(VideoItem item, CancellationToken token)
        {
            try
            {
                ...
                {
                    FileName = "aria2c.exe",
                    Arguments = $"--allow-overwrite=true --file-allocation=none -d \"{downloadPath}\" -o \"{safeName}.mp4\" \"{item.Url}\"",
        ...
        private void ParseDownloadProgress(string data, VideoItem item)
        {
            try
            {
                string downloadPattern =
                    @"\[\#(\w+)\s+([\d.]+\w*B)/([\d.]+\w*B)\((\d+)%\)\s+CN:\d+\s+DL:([\d.]+\w*B)\s+ETA:([\d]+\w*)\]";

                var downloadMatch = Regex.Match(data, downloadPattern);
                if (downloadMatch.Success)
                {
                    string downloadedSize = ConvertToDecimalSize(downloadMatch.Groups[2].Value);
                    string totalSize = ConvertToDecimalSize(downloadMatch.Groups[3].Value);
                    int progress = int.Parse(downloadMatch.Groups[4].Value);
                    string currentSpeed = ConvertToMBps(downloadMatch.Groups[5].Value);
                    string remainingTime = ConvertTimeUnitsToChinese(downloadMatch.Groups[6].Value);
            ...
            {
                AppendLog($"解析下载进度时出错: {ex.Message}");
            }
        }

        private string ConvertTimeUnitsToChinese(string timeString)
        {
            if (string.IsNullOrWhiteSpace(timeString)) return "未知";

            if (int.TryParse(timeString, out int totalSeconds))
            {
            ...
            try
            {
                if (timeString.Contains("h") && timeString.Contains("m") && timeString.Contains("s"))
                {
                    var parts = timeString.Split('h', 'm', 's');
                    if (parts.Length >= 3 &&
                ...
                else if (timeString.Contains("h") && timeString.Contains("m"))
                {
                    var parts = timeString.Split('h', 'm');
                    if (parts.Length >= 2 &&
                        int.TryParse(parts[0], out int hours) &&
                        int.TryParse(parts[1], out int minutes))
                    {
                ...
                else if (timeString.Contains("m") && timeString.Contains("s"))
                {
                ...
                {
                    if (int.TryParse(timeString.TrimEnd('h'), out int hours))
                        return $"{hours}小时";
                }
                else if (timeString.EndsWith("m"))
                {
                    if (int.TryParse(timeString.TrimEnd('m'), out int minutes))
                        return $"{minutes}分";
                }
                else if (timeString.EndsWith("s"))
                {
                    if (int.TryParse(timeString.TrimEnd('s'), out int seconds))
                        return $"{seconds}秒";
                }
            }
            catch { }
        ...
        private string ConvertToMBps(string speedWithUnit)
        {
            ...
            switch (unit)
            {
                ...
                case "MIB": valueInMBps = value; break;
                case "GIB": valueInMBps = value * 1024; break;
        ...
        {
            if (string.IsNullOrEmpty(sizeWithUnit)) return sizeWithUnit;
        ...
        private void InitializeTool_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                var configDialog = new MaxDownloadConfigDialog(_maxConcurrentDownloads);
                if (configDialog.ShowDialog() == true)
                {
                    _maxConcurrentDownloads = configDialog.SelectedMaxDownloads;
                    _downloadSemaphore = new SemaphoreSlim(_maxConcurrentDownloads);
                    AppendLog($"已设置最大同时下载数量为: {_maxConcurrentDownloads}");
                }
    ...
    public class MaxDownloadConfigDialog : Window
    {
        public int SelectedMaxDownloads { get; private set; }

        public MaxDownloadConfigDialog(int currentMax)
        {
            Title = "配置最大同时下载数量";
            Width = 300;
            Height = 150;
            ...
            SelectedMaxDownloads = currentMax;

            var stackPanel = new StackPanel { Margin = new Thickness(10) };

            var label = new Label { Content = "设置最大同时下载数量:" };
            ...
            var valueLabel = new Label { Content = $"当前值: {currentMax}" };

            slider.ValueChanged += (s, e) =>
            {
                SelectedMaxDownloads = (int)slider.Value;
                valueLabel.Content = $"当前值: {SelectedMaxDownloads}";
            };
